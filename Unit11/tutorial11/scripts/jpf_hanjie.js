...
*/
window.onload = init; // NOTE no parenthesis with event handler () Do not use Init()

var puzzleCells;
var cellBackground;

function init(){
   //Insert the title of the puzzle
   ...
   for (var i = 0; i < puzzleButtons.length; i++) {
      puzzleButtons[i].onclick = swapPuzzle; //notice no parenthesis with event handler and lowercase
   }

   setupPuzzle();
   //Add an event listner for the mouseup event
   document.addEventListener("mouseup", endBackground);
   //Add an event listner to show solution
...
function setupPuzzle() {
   //find all cells in the puzzle and color gold to hide solution
   puzzleCells = document.querySelectorAll("table#hanjieGrid td");

   for (var i =0; i < puzzleCells.length; i++) {
      puzzleCells[i].style.backgroundColor = "rgb(233, 207, 29)";
      puzzleCells[i].onmousedown = setBackground;
      //use a pencil image as the defult curser when user is inside these cells 
      puzzleCells[i].style.cursor = "url(images/jpf_pencil.png), pointer";
   }

   //create object collection of all the filled and empty cells
   ...
   //create listner to highlight the incorrect cells
   document.getElementById("peek").addEventListener("click", 
   function() {
      //display the incorrect white cells in pink
   ...
   //display the incorrect white cells in red
   ...
   // remove the hints after .5 second
   setTimeout(
      function() {
...
function setBackground(e) {
   ...
   if(e.shiftKey) {
      ...
      cellBackground = "rgb(101, 101, 101)"; //grey
...
function endBackground() {
   //remove the event listner for all the cells we had it on
   for (var i=0; i < puzzleCells.length; i++) {
